...
void Print_PAS_Info(void);
void Error_Handler(void);

/* USER CODE BEGIN 0 */
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
    Read_ADC_Values(); // Read all ADC values
}
...
void Read_ADC_Values(void) {
    ...
    // Update LED brightness with potentiometer connected to PA1 (adc_values[0])
...
int main(void) {
  ...
  while (1)
  {
    ...
    // Print PAS information
    Print_PAS_Info();

    // Add a small delay to prevent overwhelming the UART
