...
from tkinter.ttk import Frame, Button, Entry, OptionMenu
import serial
...
class topFrame(Frame):
    ...
    def setUI(self):
        ...
        w = OptionMenu(settingFrame, self.comPort, *ports)
        w.pack(fill=X)

        BaudFrame = Frame(settingFrame)
        BaudFrame.pack(fill=X)
        Label(BaudFrame, text="Baud:").pack(side=LEFT)
        self.baud_entry = Entry(BaudFrame, width=15, validate="focusout", validatecommand=self.baudValidate)
        ...
        StreamFrame.pack()
        self.btnStartStream = Button(StreamFrame, text="Start Stream", command=self.startStream, state=DISABLED)
        self.btnStopStream = Button(StreamFrame, text="Stop Stream", command=self.stopStream, state=DISABLED)
        self.btnGetConfig = Button(StreamFrame, text="Get Config", command=self.getConfig, state=DISABLED)
    ...
    def sendConfig(self):
        for setting in drivesettings:
            dataToSend = b"SET " + setting.encode() + b" " + self.driveSettingsEntries[drivesettings.index(setting)].get().encode() + b"\r"
            print(dataToSend)
